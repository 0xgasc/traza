    ...
    return (
      <div className="min-h-screen bg-stone-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm font-mono text-stone-500">LOADING PROFILE...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-stone-100 flex items-center justify-center p-4">
        <div className="card max-w-sm w-full text-center shadow-brutal">
          <h1 className="text-2xl font-bold tracking-tighter uppercase mb-4">Traza</h1>
    ...
    <div className="min-h-screen bg-stone-100">
      ...
      <div className="bg-white border-b-4 border-black">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tighter uppercase">Traza</h1>
            <p className="text-xs text-stone-500 font-mono">SIGNER PROFILE</p>
          </div>
          <button
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="card shadow-brutal">
          <p className="text-xs font-semibold uppercase tracking-wide text-stone-400 mb-3">Account</p>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-xs font-mono uppercase text-stone-400">Name</span>
              <span className="font-semibold text-sm">{profile?.name}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs font-mono uppercase text-stone-400">Email</span>
              <span className="font-mono text-sm">{profile?.email}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs font-mono uppercase text-stone-400">Member Since</span>
        ...
        <div className="card shadow-brutal">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-stone-400">Saved Signature</p>
          ...
          {sigSaved && (
            <div className="mb-3 p-2 border-2 border-green-500 bg-green-50">
              <p className="text-xs font-semibold text-green-700 uppercase">Signature saved!</p>
            </div>
          )}
          ...
          {profile?.savedSignatureData && !editingSig ? (
            ...
            <div>
              <p className="text-xs text-stone-500 mb-3">Draw or type your new signature below:</p>
              <SignatureCapture onComplete={handleSaveSignature} />
              {savingSig && <p className="mt-2 text-xs font-mono text-stone-500 text-center">SAVING...</p>}
              <button
            ...
            <div>
              <p className="text-sm text-stone-500 mb-4">
                No saved signature yet. Draw or type one below and it will be pre-loaded on your next signing request.
              </p>
              <SignatureCapture onComplete={handleSaveSignature} />
              {savingSig && <p className="mt-2 text-xs font-mono text-stone-500 text-center">SAVING...</p>}
            </div>
          ...
          {profile?.savedSignatureUpdatedAt && !editingSig && (
            <p className="mt-2 text-xs text-stone-400 font-mono">
              Last updated {new Date(profile.savedSignatureUpdatedAt).toLocaleDateString("en-US", { dateStyle: "medium" })}
            </p>
          )}
        </div>
        ...
        <div className="card shadow-brutal">
          ...
          {history.length === 0 ? (
            <p className="text-sm text-stone-500">No signing history yet.</p>
          ) : (
                ...
                <div key={item.id} className="flex items-start justify-between py-3 border-b-2 border-stone-100 last:border-0">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-stone-400 font-mono mt-0.5">
                      from {item.document.owner?.name ?? "Unknown"} ·{" "}
