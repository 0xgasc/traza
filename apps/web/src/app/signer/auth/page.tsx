    ...
    async function verify() {
      try {
        ...
        if (!res.ok) {
          ...
          if (code === "TOKEN_USED") throw new Error("This link has already been used. Request a new one.");
          if (code === "TOKEN_EXPIRED") throw new Error("This link has expired. Request a new one.");
          throw new Error(d?.error?.message || "Invalid or expired link.");
        }
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex items-center justify-center p-4">
      <div className="card max-w-sm w-full text-center shadow-brutal">
        <h1 className="text-2xl font-bold tracking-tighter uppercase mb-6">Traza</h1>

        {status === "verifying" && (
          <>
            <div className="inline-block w-8 h-8 border-4 border-black border-t-transparent animate-spin mx-auto mb-4" />
            <p className="text-sm font-mono text-stone-500">VERIFYING LINK...</p>
          </>
        )}

        {status === "success" && (
          <div className="p-4 border-4 border-green-500 bg-green-50">
            <p className="font-black uppercase text-green-800 mb-1">Signed In</p>
        ...
        {status === "error" && (
          <>
            <div className="p-4 border-4 border-black bg-stone-100 mb-4">
              <p className="font-black uppercase text-sm mb-1">Link Invalid</p>
              <p className="text-xs text-stone-600">{errorMsg}</p>
            </div>
            <p className="text-xs text-stone-400 font-mono">
              Close this window and request a new sign-in link from the signing page.
            </p>
          </>
        )}
      </div>
