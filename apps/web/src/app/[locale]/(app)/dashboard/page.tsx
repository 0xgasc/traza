...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: t("totalDocuments"), value: stats?.totalDocuments ?? 0, href: "/documents" },
    { label: t("awaitingSignatures"), value: stats?.pendingSignatures ?? 0, href: "/documents?status=PENDING" },
    { label: t("signedThisMonth"), value: stats?.completedThisMonth ?? 0, href: "/documents?status=SIGNED" },
    { label: t("totalSignatures"), value: stats?.totalSignatures ?? 0, href: null },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold uppercase tracking-tight">{t("needsAttention")}</h2>
          ...
          {pendingDocs.length === 0 ? (
            <div className="card">
              <p className="text-sm text-stone-500">{t("noPending")}</p>
            </div>
          ) : (
        ...
        {/* Recent Activity */}
        <div>
          <h2 className="text-xl font-bold uppercase tracking-tight mb-4">{t("recentActivity")}</h2>
          {activity.length === 0 ? (
            <div className="card">
              <p className="text-sm text-stone-500">{t("noActivity")}</p>
            </div>
          ) : (
            <div className="border-4 border-black bg-white">
              {activity.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold leading-tight">{t(`events.${item.eventType}`)}</p>
                      {item.document && (
