...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    setSelectedDocs(new Set()); // Clear selection when filters change
  ...
  const handleBulkTag = async (tagId: string) => {
    const promises = Array.from(selectedDocs).map((docId) =>
      apiPost("/api/v1/tags/documents/" + docId, { tagId }).catch(() => {})
    );
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-stone-500 font-mono mt-1">
            {pagination ? t("subtitleWithCount", { total: pagination.total }) : t("subtitleWithCount", { total: 0 })}
          </p>
        </div>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <span className="text-xs font-semibold uppercase tracking-wide text-stone-400">{t("filterByTag")}</span>
          <button
      ...
      {showTagManager && (
        <div className="mb-6 p-4 border-4 border-black bg-white space-y-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide">{t("manageTagsTitle")}</h3>
          {/* Existing tags */}
          {tags.length > 0 && (
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-black text-white border-4 border-black shadow-brutal px-6 py-4 flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="px-4 py-2 bg-white text-black text-xs font-bold uppercase hover:bg-stone-200 transition-colors"
            >
              Tag Selected
            </button>
            {showBulkTagPicker && (
              <div className="absolute bottom-full left-0 mb-2 bg-white border-4 border-black shadow-brutal min-w-[200px] max-h-[300px] overflow-y-auto">
                {tags.length === 0 ? (
                  <div className="px-4 py-3 text-xs text-stone-500">No tags available</div>
                ) : (
                  tags.map((tag) => (
                    <button
                      key={tag.id}
                      onClick={() => handleBulkTag(tag.id)}
                      className="flex items-center gap-2 w-full px-4 py-3 text-xs font-bold uppercase hover:bg-stone-100 text-left text-black border-b-2 border-stone-100 last:border-b-0"
                    >
                      <span
                        className="w-3 h-3 rounded-full flex-shrink-0"
                        style={{ backgroundColor: tag.color }}
          ...
          <button
            ...
            className="px-4 py-2 border-2 border-white text-xs font-bold uppercase hover:bg-white hover:text-black transition-colors"
          >
            Clear
          </button>
        </div>
      ...
      {documents.length === 0 && !loading ? (
        <div className="card text-center py-12">
          <p className="text-stone-500 font-semibold">{t("noDocuments")}</p>
