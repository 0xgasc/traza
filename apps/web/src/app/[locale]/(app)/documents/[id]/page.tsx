...
export default function DocumentDetailPage() {
  ...
  const handleAction = async (action: string) => {
    ...
    try {
      if (action === "download") {
        ...
        a.click();
        URL.revokeObjectURL(url);
        showToast(t("proofDownloaded"));
      } else if (action === "verify") {
        const result = await apiGet<{ verified: boolean; documentHash: string; hashAlgorithm: string }>("/api/v1/documents/" + id + "/verify");
        if (result.verified) {
          showToast(`${t("integrityVerified")} — ${result.hashAlgorithm}: ${result.documentHash.slice(0, 16)}…`, true);
        } else {
          showToast(t("integrityFailed"), false);
        }
      } else if (action === "void") {
  ...
  const handleSaveAsTemplate = async () => {
    const templateName = prompt(t("templateNamePrompt"), doc?.title);
    ...
    try {
      ...
      showToast(t("templateFailed"), false);
    }
  };
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            {doc.expiresAt && doc.status === "PENDING" && (
              <span className="text-xs font-mono text-yellow-700 bg-yellow-50 border border-yellow-300 px-2 py-0.5">
                {t("expires")} {new Date(doc.expiresAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
              </span>
            )}
            {doc.voidReason && (
              <span className="text-xs font-mono text-red-700">
                {t("reason")}: {doc.voidReason}
              </span>
            )}
          </div>
        </div>
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                showToast(err instanceof Error ? err.message : t("resendFailed"), false);
              } finally {
                setActionLoading("");
              }
            }}
