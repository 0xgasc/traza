    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-stone-500 font-mono mt-1">
            {template?._count?.fields ?? template?.fields?.length ?? 0} {t("statsFields").toUpperCase()} · {t("usedCount", { count: template?.useCount ?? 0 }).toUpperCase()}
          </p>
        </div>
        <Link href="/templates" className="btn-secondary">{t("allTemplates")}</Link>
      </div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="max-w-lg space-y-4 mb-8 p-6 border-4 border-black bg-white">
          <h2 className="text-sm font-semibold uppercase tracking-wide">{t("editFormTitle")}</h2>
          <div>
      ...
      <div className="max-w-lg space-y-4">
        {template?.description && (
          <div className="card">
            <p className="text-xs text-stone-500 font-mono uppercase mb-1">{t("descriptionLabel")}</p>
        ...
        {signerRoles.length > 0 && (
          <div className="card">
            <p className="text-xs text-stone-500 font-mono uppercase mb-3">{t("signerRoles")}</p>
        ...
        <div className="card">
          <p className="text-xs text-stone-500 font-mono uppercase mb-3">{t("stats")}</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-black">{template?._count?.fields ?? template?.fields?.length ?? 0}</p>
              <p className="text-xs font-mono text-stone-500">{t("statsFields").toUpperCase()}</p>
            </div>
            <div>
              <p className="text-2xl font-black">{signerRoles.length}</p>
              <p className="text-xs font-mono text-stone-500">{t("statsRoles").toUpperCase()}</p>
            </div>
            <div>
              <p className="text-2xl font-black">{template?.useCount}</p>
              <p className="text-xs font-mono text-stone-500">{t("statsUses").toUpperCase()}</p>
            </div>
          </div>
        </div>
      </div>
