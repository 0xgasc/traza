  ...
  if (error) {
    return (
      <div className="bg-red-100 border-4 border-red-600 p-6">
        <h2 className="text-lg font-bold text-red-800 uppercase">{t('error')}</h2>
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <div className="col-span-2 space-y-8">
          ...
          <div className="bg-white border-4 border-black p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            {isEditing ? (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold uppercase mb-2">{t('fieldName')}</label>
                  <input
                    type="text"
                    value={editName}
                ...
                <div>
                  <label className="block text-sm font-bold uppercase mb-2">{t('fieldBillingEmail')}</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-sm font-bold uppercase mb-2">{t('fieldPlanTier')}</label>
                  <select
                    value={editPlanTier}
                ...
                <div>
                  <label className="block text-sm font-bold uppercase mb-2">{t('fieldBrandColor')}</label>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div>
                    <p className="text-xs font-bold uppercase text-zinc-500">{t('fieldBillingEmail')}</p>
                    <p className="font-mono">{org.billingEmail || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase text-zinc-500">{t('fieldBrandColor')}</p>
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase text-zinc-500">{t('fieldCreated')}</p>
                    <p>{new Date(org.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase text-zinc-500">{t('fieldLastUpdated')}</p>
          ...
          <div className="bg-white border-4 border-black p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            <h2 className="text-xl font-black uppercase tracking-tighter mb-6">
              {t('membersHeading', { count: org._count.members })}
            </h2>
            <div className="space-y-3">
              {org.members.map((member) => (
                ...
                >
                  <div>
                    <p className="font-bold">{member.user.name || t('unnamed')}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-zinc-400">
                      {t('joined', { date: new Date(member.joinedAt).toLocaleDateString() })}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border-4 border-black p-6 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold uppercase text-zinc-500">{t('statMembers')}</span>
                <span className="text-2xl font-black">{org._count.members}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold uppercase text-zinc-500">{t('statDocuments')}</span>
          ...
          <div className="bg-red-50 border-4 border-red-600 p-6">
            ...
            <button
              onClick={async () => {
                const input = prompt(t('confirmDeletePrompt', { slug: org.slug }));
