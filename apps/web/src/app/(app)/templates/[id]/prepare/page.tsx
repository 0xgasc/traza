  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tighter uppercase">Prepare Template</h1>
          <p className="text-sm font-mono text-stone-500 mt-1">{template.name}</p>
        </div>
        <Link href={'/templates/' + id} className="btn-secondary">Back</Link>
      </div>
      ...
      <div className="border-4 border-black bg-white p-6 mb-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
        ...
        <p className="text-xs text-stone-500 font-mono mb-4">
          Define roles (e.g. &ldquo;Customer&rdquo;, &ldquo;Witness&rdquo;) instead of emails — assign emails when you send.
        </p>
        ...
        <div className="flex gap-3 items-end">
          <div className="flex-1">
            <label className="block text-xs font-bold uppercase tracking-widest text-stone-500 mb-1">Role Name</label>
            <input
              type="text"
              value={newRole}
      ...
      <div className="mt-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {saveStatus === 'saved' && <span className="text-sm font-semibold text-green-600 uppercase">Saved!</span>}
          {saveStatus === 'error' && <span className="text-sm font-semibold text-red-600 uppercase">Save failed</span>}
        </div>
        <button
